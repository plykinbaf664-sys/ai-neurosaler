...
export function buildNeiroPrompt(params: BuildNeiroPromptParams) {
  ...
  const neuroauditLeadRules = [
    "GOAL: быстро понять, есть ли у человека релевантная ситуация для нейроаудита, и если да, мягко довести до записи.",
    "- сначала оцени, есть ли вообще смысл в нейроаудите; если нет, не дави.",
    "- материалы не обязательны: если их нет, это нормальный сценарий.",
    "- если человек шутит или уводит в сторону, коротко отреагируй по контексту и мягко верни к теме.",
    "- если человек спрашивает про продукты, формат работы, цену, материалы или как всё устроено, сначала ответь по сути из данных, а уже потом мягко веди к записи.",
    "- отвечай на прямые вопросы по сути, а не шаблоном.",
    "- если человек согласился на аудит, сразу дай ссылку на календарь один раз и не переспрашивай.",
    "- если calendarLink отсутствует, не ломайся: скажи, что Александр свяжется лично.",
  ];
  ...
  return [
    ...
    "- если stage=handoff, не запускай продажу заново, но прямые вопросы всё равно отвечай по сути, используя данные из DIALOGUE, OFFERS, FAQ и OBJECTIONS.",
    ...
    `- если ссылка на календарь уже была отправлена раньше или stage уже booked/confirmed, не отправляй ${calendarLink} повторно без запроса. Если клиент просит ссылку снова, отправь её ещё раз.`,
